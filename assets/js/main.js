...
function darkHandler() {
  ...
  if (darkButtonIsClickOrNot === false) {
    ...
    for (var i = 0; i < 3; i++) {
      svg[i].classList.remove("darkmode");
    }
    swap.classList.remove("darkmode");
    header_text.classList.remove("darkmode");
    header[0].classList.remove("darkmodeheader");

    console.log("white");

    darkButton.innerHTML = icon;
    darkButtonIsClickOrNot = false;
    icon =
